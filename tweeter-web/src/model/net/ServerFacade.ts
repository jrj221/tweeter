...
export class ServerFacade {
	...
	public async getMoreFollowees(request: PagedUserItemRequest): Promise<[User[], boolean]> {
		...
		const items: User[] | null =
			response.success && response.items ? response.items.map((dto) => User.fromDTO(dto) as User) : null;
	...
	public async getMoreStoryItems(request: PagedStatusItemRequest): Promise<[Status[], boolean]> {
		...
		const items: Status[] | null =
			response.success && response.items ? response.items.map((dto) => Status.fromDTO(dto) as Status) : null;
